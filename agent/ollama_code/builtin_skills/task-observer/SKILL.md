...
## Locus runtime integration

Locus provides encrypted, app-managed observation storage. This section
replaces the file-writing mechanics in **Session Start Protocol**, **How to
Log**, **Archival on Write**, and the workspace-path examples below:

- Never create or edit `skill-observations/` inside the user's repository.
- Use `record_skill_observation` as soon as a reusable, evidence-backed insight
  appears. Supply Issue, Suggested improvement, and Principle separately.
- Use `record_skill_observation` with `checkpoint_only: true` at mandatory
  no-observation checkpoints. Locus performs numbering, concurrency control,
  encryption, retention, and workspace scoping.
- Do not auto-edit any skill. The Settings review queue is the approval gate.
- Treat a successful native tool result as the required write and survival
  check. The shell snippets below document the portable upstream fallback and
  must not be run in Locus.

The original file protocol remains in this bundle for portability and
methodology fidelity, but this native integration has precedence while running
inside Locus.
